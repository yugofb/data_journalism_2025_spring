...
**Answer** The data has columns for precinct id number, precinct name, congress, legislature, council, school, polling id, shape length, shape area and shape.
...
**Answer** The data contains shapes for schools, churches, community centers, etc. These might be educational precincts, or polling precincts.
...
**Answer** The code creates a map compiling all of the shape data into a single map.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output confirms that the library tigrus has been installed, which means we can pull shape files directly from the U.S. census.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output includes the names of all U.S. counties from 2022, information about said counties, and their shape files.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output has the same columns as the previous code, but it only includes counties from Maryland.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output mapped all of the Maryland counties with longitude and latitude lines on the map.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** `geom_sf(data=md_counties |> filter(COUNTYFP == "033"))` specified that the output should only include counties in Maryland and then filters for only Prince George's county.`geom_sf(data=pg_precincts)` specifies that the output should be pulling precinct data from PG county.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The code created a dataset that filters for polling information in Prince George's county, groups those results by precinct, and then displays those results by the total amount made per voting type.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output is a dataset that combines the shapefiles for each precinct in PG county with the sum of votes per voting type in each precinct.

```{r}
pg_precincts_with_turnout <- pg_precincts |>
  left_join(pg_turnout, by=c("PRECINCT_ID"="precinct"))
```
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output displays the amount of votes per PG precinct via a color legend scale. Some precincts are left blank.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output is a table displaying the amount of registration voters in each Maryland county, arranged by highest total number of voters to least.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output is a dataset of estimated population data per county in Maryland.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The new output has the same county and population data as the last one, but it now includes shapefiles for each county.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output shows that the two datasets have completely different names for the column containing counties as well as different variable names for said counties. The datasets cannot be joined because of these differences.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output chaned the `NAME` column to `County`, renamed the `estimate` column to `population` and removed changes the names of the counties by removing extra information.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The output is a new dataset that combines the population data, voting data and shapefiles of the two starting datasets.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The new output has fixed the display issue with the previous code, which caused some counties to have their voting data displayed as NA.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The new output adds an additional column that calculates the amount of total voters per 10k people.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The code plots `voters_per_10k` by using each county's shapefile. The density of average voters per 10k people in each county is designated by a color legend scale.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The new output uses the "magma" viridis fill option. Instead of a scale, the colors are distinguished by a range. The colors on this map are more distinct than the previous chart.
...
**Answer** The new output has some degree of color scaling, which helps to distinguish counties with similar voter averages.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The code adds a new column that shows the average amount of unafilliated voters per 10000 voters for each county.
...
**Answer** The code creates a map of the average number of voters per 10k people for each Maryland county. The legend for this chart is smaller because the amount of unafilliated voters is much less than the number of total voters. There is also less of a number range between all of the counties.
