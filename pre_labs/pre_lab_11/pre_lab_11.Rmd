...
```{r}
##easier way to read in the html

pres_url <- read_html(URL)
...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns represent the number of votes given to each party by Maryland jusisdiction, i.e. county. However, the votes for each candidate should be in `<dbl>` and not `<chr>`. This may have occured because the numbers have commas, which cannot be present in the `<dbl>` datatype.
...
**Answer** I think the numbers for rename may represent the order in which candidates were listed on the original ballot.
...
**Answer** There are 20 <li> (listed) tags in the source's html. These twenty tags are dicided between  seven <ul> (unlisted) tags. This suggests that the individual <li> tags are being categorized depending on their content.

```{r}

cmte_results |> html_elements('ul')
...
**Answer** The output shows the text inside of each <li> tag. This text functions as a title and tells us the contents of each <li>.
...
**Answer** The result is a list with a length of 54 items that each contain the item's html text and href (urls), listed as "type" and "value."
